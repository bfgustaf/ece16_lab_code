  ...
  if (confirmedCentral) {                              // This implicitly checks if BLE is asleep (confirmedCentral would be false if asleep==true)
    ...
    if (endTime - startTime >= period_us) {
      ...
      if (imuDataReady) {
        ...
        dtostrf(ax, 6, 0, imu_text);                           // Write the IMU x axis data to the IMU data char array
        sprintf(out_text, "%s, %s;", time_text, imu_text);     // Combine char arrays and add coma, semicolon
